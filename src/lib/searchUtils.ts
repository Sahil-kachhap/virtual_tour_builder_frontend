...
const samplePoints = [
  { id: 1, name: 'Eiffel Tower', lat: 48.8584, lng: 2.2945, category: 'landmark' },
  { id: 2, name: 'Louvre Museum', lat: 48.8606, lng: 2.3376, category: 'museum' },
  { id: 3, name: 'Notre-Dame Cathedral', lat: 48.8530, lng: 2.3499, category: 'historical' },
  { id: 4, name: 'Arc de Triomphe', lat: 48.8738, lng: 2.2950, category: 'landmark' },
  { id: 5, name: 'Sacré-Cœur', lat: 48.8867, lng: 2.3431, category: 'religious' },
  { id: 6, name: 'Paris Walking Tour', lat: 48.8566, lng: 2.3522, category: 'tour' },
  { id: 7, name: 'Montmartre Art Tour', lat: 48.8867, lng: 2.3431, category: 'tour' },
  { id: 8, name: 'Seine River Cruise', lat: 48.8584, lng: 2.2945, category: 'tour' },
  { id: 9, name: 'Paris Food Tour', lat: 48.8606, lng: 2.3376, category: 'tour' },
  { id: 10, name: 'Luxembourg Gardens', lat: 48.8462, lng: 2.3371, category: 'park' },
];
...
export function initPlacesService(map: google.maps.Map) {
  ...
  console.warn("Failed to initialize Places services - Google Maps not fully loaded");
  return false;
}
...
function mapPredictionToSearchResult(prediction: google.maps.places.AutocompletePrediction, index: number): SearchResult {
...
export const debouncedSearchLocations = debounce(
  (
    ...
    if (!window.google || !window.google.maps || !window.google.maps.places || !autocompleteService) {
      console.warn("Using sample data for search as Google Places API is not available");
      const results = samplePoints.filter(point => 
